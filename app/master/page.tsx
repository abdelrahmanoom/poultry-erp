...
export default function MasterDashboardPage() {
  ...
  const toggleProtect = async (t: any) => {
    const newState = !t.is_protected;
    if (newState) {
      if (!confirm('حماية النشاط تمنع مسحه من Master.\n\nمتابعة؟')) return;
    }
    try {
      const res = await fetch('/api/master/tenants/' + t.id + '/protect', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ is_protected: newState }),
      });
      const data = await res.json();
      if (!res.ok || !data.success) {
        showToast('خطأ: ' + (data.error || 'فشل'), 'error');
        return;
      }
      showToast(newState ? 'تم تفعيل الحماية' : 'تم إلغاء الحماية');
      load();
    } catch (e: any) {
        ...
        {sandbox && (
          <div className="bg-gradient-to-br from-amber-50 to-amber-100 border-2 border-amber-300 rounded-3xl p-6 space-y-4">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button onClick={() => toggleProtect(sandbox)} className="bg-white hover:bg-amber-50 text-amber-800 font-bold px-4 py-2.5 rounded-xl text-xs flex items-center gap-1.5 border border-amber-300">{sandbox.is_protected ? <Unlock className="w-3.5 h-3.5" /> : <Lock className="w-3.5 h-3.5" />}<span>{sandbox.is_protected ? 'إلغاء الحماية' : 'حماية'}</span></button>
            ...
            {realTenants.map((t) => (
              <div key={t.id} className={'bg-white p-5 rounded-3xl border-2 transition space-y-3 ' + (t.is_read_only ? 'border-amber-300 bg-amber-50/30' : 'border-slate-200 hover:border-slate-400')}>
                <div className="flex justify-between items-start">
                  <div className="flex-1 min-w-0">
                    <h3 className="text-base font-black text-slate-900 truncate flex items-center gap-2">
                      ...
                      {t.is_protected && <span className="text-[9px] bg-emerald-200 text-emerald-900 px-1.5 py-0.5 rounded font-bold">🔒 محمي</span>}
                    </h3>
                ...
                <div className="grid grid-cols-2 gap-2 pt-2 border-t">
                  ...
                  <button onClick={() => toggleProtect(t)} className={(t.is_protected ? 'bg-emerald-100 hover:bg-emerald-200 text-emerald-800' : 'bg-slate-100 hover:bg-slate-200 text-slate-700') + ' font-bold py-2 rounded-xl text-xs flex items-center justify-center gap-1'}>{t.is_protected ? <Unlock className="w-3 h-3" /> : <Lock className="w-3 h-3" />}<span>{t.is_protected ? 'إلغاء' : 'حماية'}</span></button>
                  ...
                  <button onClick={() => { if (t.is_protected) { showToast('النشاط محمي — أزل الحماية أولاً', 'error'); return; } setWipingTenant(t); setWipeConfirmText(''); }} className={'font-bold py-2 rounded-xl text-xs flex items-center justify-center gap-1 col-span-2 ' + (t.is_protected ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-rose-50 hover:bg-rose-100 text-rose-700')} disabled={t.is_protected}><Trash2 className="w-3 h-3" /><span>مسح البيانات</span></button>
                </div>
              </div>
            ))}
          </div>
