                    ...
                    return (
                      <div key={t.code} className={'p-4 rounded-2xl border-2 transition flex flex-wrap gap-3 items-center ' + (hasValue ? 'bg-emerald-50/60 border-emerald-300' : 'bg-white border-slate-200')}>
                        ...
                        <div>
                          ...
                          <input type="number" step="0.01" value={t.balance} disabled={openingLocked} onChange={(e) => { const u = [...openTreasuries]; u[idx].balance = Number(e.target.value); setOpenTreasuries(u); }} placeholder="0" className={'w-40 border-2 rounded-xl px-3 text-sm font-bold font-mono h-11 ' + (hasValue ? 'border-emerald-300 bg-white text-emerald-900' : 'border-slate-300 disabled:bg-slate-100')} />
                        </div>
                      </div>
                    );
                  })}
                      ...
                      <div key={idx} className={'p-3 rounded-2xl border-2 transition space-y-2 ' + (hasValue ? 'bg-emerald-50/60 border-emerald-300' : 'bg-white border-slate-200')}>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-2 items-end">
                          <div>
                            ...
                            <input type="text" value={c.name} disabled={openingLocked} onChange={(e) => { const u = [...openCustomers]; u[idx].name = e.target.value; setOpenCustomers(u); }} placeholder="اسم العميل" className={'w-full border rounded-xl px-3 text-xs font-bold h-10 ' + (hasValue ? 'border-emerald-300 bg-white text-emerald-900' : 'border-slate-200 disabled:bg-slate-100')} />
                          </div>
                          <div>
                            <label className={'block text-[10px] font-bold mb-0.5 ' + (hasValue ? 'text-emerald-700' : 'text-slate-500')}>الهاتف</label>
                            <input type="text" value={c.phone} disabled={openingLocked} onChange={(e) => { const u = [...openCustomers]; u[idx].phone = e.target.value; setOpenCustomers(u); }} placeholder="الهاتف" className={'w-full border rounded-xl px-3 text-xs font-bold h-10 ' + (hasValue ? 'border-emerald-300 bg-white text-emerald-900' : 'border-slate-200 disabled:bg-slate-100')} />
                          </div>
                          <div className="flex gap-2 items-end">
                            <div className="flex-1">
                              ...
                              <input type="number" value={c.balance} disabled={openingLocked} onChange={(e) => { const u = [...openCustomers]; u[idx].balance = Number(e.target.value); setOpenCustomers(u); }} placeholder="0" className={'w-full border-2 rounded-xl px-3 text-xs font-bold font-mono h-10 ' + (hasValue ? 'border-emerald-300 bg-white text-emerald-900' : 'border-slate-300 disabled:bg-slate-100')} />
                            </div>
                      ...
                      <div key={idx} className={'p-3 rounded-2xl border-2 transition space-y-2 ' + (hasValue ? 'bg-emerald-50/60 border-emerald-300' : 'bg-white border-slate-200')}>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-2 items-end">
                          <div>
                            ...
                            <input type="text" value={s.name} disabled={openingLocked} onChange={(e) => { const u = [...openSuppliers]; u[idx].name = e.target.value; setOpenSuppliers(u); }} placeholder="اسم المورد" className={'w-full border rounded-xl px-3 text-xs font-bold h-10 ' + (hasValue ? 'border-emerald-300 bg-white text-emerald-900' : 'border-slate-200 disabled:bg-slate-100')} />
                          </div>
                          <div>
                            <label className={'block text-[10px] font-bold mb-0.5 ' + (hasValue ? 'text-emerald-700' : 'text-slate-500')}>الهاتف</label>
                            <input type="text" value={s.phone} disabled={openingLocked} onChange={(e) => { const u = [...openSuppliers]; u[idx].phone = e.target.value; setOpenSuppliers(u); }} placeholder="الهاتف" className={'w-full border rounded-xl px-3 text-xs font-bold h-10 ' + (hasValue ? 'border-emerald-300 bg-white text-emerald-900' : 'border-slate-200 disabled:bg-slate-100')} />
                          </div>
                          <div className="flex gap-2 items-end">
                            <div className="flex-1">
                              <label className={'block text-[10px] font-bold mb-0.5 ' + (hasValue ? 'text-emerald-700' : 'text-slate-500')}>المستحق الافتتاحي (ج)</label>
                              <input type="number" value={s.balance} disabled={openingLocked} onChange={(e) => { const u = [...openSuppliers]; u[idx].balance = Number(e.target.value); setOpenSuppliers(u); }} placeholder="0" className={'w-full border-2 rounded-xl px-3 text-xs font-bold font-mono h-10 ' + (hasValue ? 'border-emerald-300 bg-white text-emerald-900' : 'border-slate-300 disabled:bg-slate-100')} />
                            </div>
                        ...
                        <div key={st.code} className={'p-3 rounded-2xl border-2 space-y-2 transition ' + (hasValue ? 'bg-emerald-50/60 border-emerald-300' : 'bg-white border-slate-200')}>
                          ...
                          <div className="grid grid-cols-3 gap-2">
                            <div>
                              ...
                              <input type="number" step="0.1" value={st.weight} disabled={openingLocked} onChange={(e) => { const u = [...openStock]; u[idx].weight = Number(e.target.value); setOpenStock(u); }} placeholder="0" className={'w-full border rounded-lg px-2 h-9 text-xs font-mono ' + (hasValue ? 'border-emerald-300 bg-white font-bold text-emerald-900' : 'border-slate-200 disabled:bg-slate-100')} />
                            </div>
                            <div>
                              <label className={'block text-[10px] font-bold mb-0.5 ' + (hasValue ? 'text-emerald-700' : 'text-slate-500')}>تكلفة/كجم (ج)</label>
                              <input type="number" value={st.cost} disabled={openingLocked} onChange={(e) => { const u = [...openStock]; u[idx].cost = Number(e.target.value); setOpenStock(u); }} placeholder="0" className={'w-full border rounded-lg px-2 h-9 text-xs font-mono ' + (hasValue ? 'border-emerald-300 bg-white font-bold text-emerald-900' : 'border-slate-200 disabled:bg-slate-100')} />
                            </div>
                            <div>
