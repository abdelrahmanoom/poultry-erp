...
export default function SettingsPage() {
  ...
  const handleSaveOpening = async (e: any) => {
    e.preventDefault();
    if (openingLocked) { showToast('الأرصدة مقفلة', 'error'); return; }
    let saved = 0;
    ...
    for (const c of openCustomers) {
      if (c.name.trim() && Number(c.balance) > 0) {
        ...
        await supabase.from('customers').insert([{ name: c.name.trim(), phone: c.phone.trim(), balance: Number(c.balance), is_active: true }]);
        saved++;
      }
    }
    ...
    for (const s of openSuppliers) {
      if (s.name.trim() && Number(s.balance) > 0) {
        ...
        await supabase.from('suppliers').insert([{ name: s.name.trim(), phone: s.phone.trim(), balance: Number(s.balance), is_active: true }]);
        saved++;
      }
    }
    ...
    for (const st of openStock) {
      if (Number(st.weight) > 0) {
        ...
        await supabase.from('inventory_lots').insert([{
          product_code: st.code,
          source_type: 'opening',
          source_ref: 'OPENING-BALANCE',
          quantity_kg: Number(st.weight),
          remaining_kg: Number(st.weight),
          cost_per_kg: Number(st.cost) || 0
        }]);
    ...
    showToast('تم اعتماد ' + saved + ' حركة افتتاحية وقفلها');
    loadData();
  };
          ...
          <div className="space-y-6">
            <form onSubmit={handleAddProduct} className="bg-slate-50 p-6 rounded-3xl border space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs font-bold">
                <div><label className="block text-slate-700 mb-1.5">الاسم:</label><input type="text" value={pName} onChange={(e) => setPName(e.target.value)} placeholder="دبابيس صافي" className="w-full border rounded-xl px-3 bg-white h-11" required /></div>
            ...
            {showNewTreasuryForm && (
              <form onSubmit={handleAddTreasury} className="bg-slate-50 p-5 rounded-2xl border space-y-3">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
                  <div><label className="block text-xs font-bold text-slate-700 mb-1.5">الاسم:</label><input type="text" value={newTreasuryName} onChange={(e) => setNewTreasuryName(e.target.value)} placeholder="عهدة سيارة التوزيع" className="w-full border-2 rounded-xl px-4 text-sm font-bold bg-white h-11" required /></div>
        ...
        {activeTab === 'opening' && (
          <div className="space-y-5">
            ...
            <form onSubmit={handleSaveOpening} className="space-y-5">
              {openingSubTab === 'treasuries' && (
                <div className="bg-slate-50 p-5 rounded-3xl border space-y-3">
                  ...
                  {openTreasuries.map((t, idx) => (
                    <div key={t.code} className="bg-white p-4 rounded-2xl border flex flex-wrap gap-3 items-center">
                      <div className="flex-1 min-w-[200px]">
                        <div className="flex items-center gap-2 mb-1">
                          {t.type === 'حساب بنكي' ? <Building2 className="w-4 h-4 text-slate-700" /> : t.type === 'عهدة' ? <UserCheck className="w-4 h-4 text-slate-700" /> : <Wallet className="w-4 h-4 text-slate-700" />}
                          <span className="text-sm font-bold text-slate-800">{t.name}</span>
                          <span className="text-[10px] text-slate-400 font-mono">{t.code}</span>
                        </div>
                        <span className="text-[11px] text-slate-500 font-bold">{t.type}</span>
                      </div>
                      <input type="number" step="0.01" value={t.balance} disabled={openingLocked} onChange={(e) => { const u = [...openTreasuries]; u[idx].balance = Number(e.target.value); setOpenTreasuries(u); }} placeholder="الرصيد (ج)" className="w-40 border-2 border-slate-300 rounded-xl px-3 text-sm font-bold font-mono h-11 disabled:bg-slate-100" />
                    </div>
                  ))}
                </div>
              )}
              ...
              {openingSubTab === 'customers' && (
                <div className="bg-slate-50 p-5 rounded-3xl border space-y-3">
                  ...
                  {openCustomers.map((c, idx) => (
                    <div key={idx} className="flex flex-wrap gap-2 items-center bg-white p-2.5 rounded-2xl border">
                      <input type="text" value={c.name} disabled={openingLocked} onChange={(e) => { const u = [...openCustomers]; u[idx].name = e.target.value; setOpenCustomers(u); }} placeholder="اسم العميل" className="flex-1 min-w-[150px] border rounded-xl px-3 text-xs font-bold h-10 disabled:bg-slate-100" />
                      <input type="text" value={c.phone} disabled={openingLocked} onChange={(e) => { const u = [...openCustomers]; u[idx].phone = e.target.value; setOpenCustomers(u); }} placeholder="الهاتف" className="w-36 border rounded-xl px-3 text-xs font-bold h-10 disabled:bg-slate-100" />
                      <input type="number" value={c.balance} disabled={openingLocked} onChange={(e) => { const u = [...openCustomers]; u[idx].balance = Number(e.target.value); setOpenCustomers(u); }} placeholder="المديونية" className="w-36 border-2 border-slate-300 rounded-xl px-3 text-xs font-bold font-mono h-10 disabled:bg-slate-100" />
                      {!openingLocked && openCustomers.length > 1 && <button type="button" onClick={() => setOpenCustomers(openCustomers.filter((_, i) => i !== idx))} className="text-rose-600 p-2"><Trash2 className="w-4 h-4" /></button>}
                    </div>
                  ))}
                </div>
              )}
              ...
              {openingSubTab === 'suppliers' && (
                <div className="bg-slate-50 p-5 rounded-3xl border space-y-3">
                  ...
                  {openSuppliers.map((s, idx) => (
                    <div key={idx} className="flex flex-wrap gap-2 items-center bg-white p-2.5 rounded-2xl border">
                      <input type="text" value={s.name} disabled={openingLocked} onChange={(e) => { const u = [...openSuppliers]; u[idx].name = e.target.value; setOpenSuppliers(u); }} placeholder="اسم المورد" className="flex-1 min-w-[150px] border rounded-xl px-3 text-xs font-bold h-10 disabled:bg-slate-100" />
                      <input type="text" value={s.phone} disabled={openingLocked} onChange={(e) => { const u = [...openSuppliers]; u[idx].phone = e.target.value; setOpenSuppliers(u); }} placeholder="الهاتف" className="w-36 border rounded-xl px-3 text-xs font-bold h-10 disabled:bg-slate-100" />
                      <input type="number" value={s.balance} disabled={openingLocked} onChange={(e) => { const u = [...openSuppliers]; u[idx].balance = Number(e.target.value); setOpenSuppliers(u); }} placeholder="المستحق" className="w-36 border-2 border-slate-300 rounded-xl px-3 text-xs font-bold font-mono h-10 disabled:bg-slate-100" />
                      {!openingLocked && openSuppliers.length > 1 && <button type="button" onClick={() => setOpenSuppliers(openSuppliers.filter((_, i) => i !== idx))} className="text-rose-600 p-2"><Trash2 className="w-4 h-4" /></button>}
                    </div>
                  ))}
                </div>
              )}
                ...
                <div className="bg-slate-50 p-5 rounded-3xl border space-y-3">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                    {openStock.map((st, idx) => (
                      <div key={st.code} className="bg-white p-3 rounded-2xl border space-y-1">
                        <span className="text-xs font-bold text-slate-700 block">{st.name}:</span>
                        <div className="flex gap-2">
                          <input type="number" step="0.1" value={st.weight} disabled={openingLocked} onChange={(e) => { const u = [...openStock]; u[idx].weight = Number(e.target.value); setOpenStock(u); }} placeholder="الوزن" className="w-1/2 border rounded-lg px-2 h-9 text-xs font-mono disabled:bg-slate-100" />
                          <input type="number" value={st.cost} disabled={openingLocked} onChange={(e) => { const u = [...openStock]; u[idx].cost = Number(e.target.value); setOpenStock(u); }} placeholder="التكلفة" className="w-1/2 border rounded-lg px-2 h-9 text-xs font-mono disabled:bg-slate-100" />
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {!openingLocked && (
                <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-2xl shadow-xl text-sm">
                  اعتماد وقفل الأرصدة الافتتاحية
                </button>
              )}
            </form>
          </div>
